...
```{r simple portfolio optimization using simulation, warning = FALSE, message = FALSE}

# Portfolio specification object using asset_names
port_spec_base = portfolio.spec(assets = colnames(index_returns))%>%
  add.objective(portfolio = port_spec_base, type = "return", name = "mean") %>%
  add.objective(type="risk_budget", name="ETL",
# Expected tail loss with a confidence level  0.95;  not asset can contribute more than 21% to total portfolio risk.
                arguments=list(p=0.95), max_prisk=0.21, enabled=FALSE)%>%
  add.constraint(type="leverage",min_sum=0.99, max_sum=1.01) %>%
  add.constraint(type="box", min=0, max=1)
```

```{r port simulation, warning = FALSE, message = FALSE}
...
#risk returns of each simulation

#combine all portfolio
base_simulation <- list("sample"=as_tibble(base_port_1), "simplex"=as_tibble(base_port_2),
            "grid"=as_tibble(base_port_3)) %>% bind_rows(.id="Method")


#simulation performance
opt.outputMvo(base_port_1,
              index_returns,
              digits = NULL,
              annualize = TRUE,
              frequency = "weekly",
